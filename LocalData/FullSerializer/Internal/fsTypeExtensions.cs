...
namespace FullSerializer
{
    public static class fsTypeExtensions
    {
        ...
        {
            string name = CSharpName(type, includeNamespace);
        ...
        public static string CSharpName(this Type type, bool includeNamespace)
        {
            ...
            IEnumerable<Type> genericArguments = (IEnumerable<Type>) type.GetGenericArguments();
            if (type.IsNested)
            {
            ...
            {
                int genericsTic = type.Name.IndexOf('`');
                if (genericsTic > 0)
                {
